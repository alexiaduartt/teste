...
// Mapeamento de campos e lógica para extrair os dados corretos
...
const LABEL_TRANSLATION: { [key: string]: string } = {
  name: 'NOME', category: 'CATEGORIA', price: 'PREÇO UNITÁRIO', 
  status: 'STATUS', quantity: 'QUANTIDADE', supplier: 'FORNECEDOR', 
  storage: 'LOCAL DE ARMAZENAMENTO', validity: 'DATA DE VALIDADE', 
  description: 'DESCRIÇÃO', type: 'TIPO', unit: 'UNIDADE', 
  notes: 'OBSERVAÇÕES', area: 'ÁREA (HECTARES)', 
  responsible: 'RESPONSÁVEL', location: 'LOCALIZAÇÃO', 
        ...
        <ScrollView style={styles.detailsContainer} contentContainerStyle={styles.detailsContent}>
          <View style={[styles.detailsGroup, { backgroundColor: themeColors.detailBackground }]}>
            {itemData ? (
                ...
                <Text style={{ color: themeColors.detailLabel }}>{`${entityType} com ID ${entityId} não encontrado.`}</Text>
            )}
          </View>
